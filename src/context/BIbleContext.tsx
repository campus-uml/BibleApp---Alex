...
import { getBibles, getChapters, loadChapterVersesFromAPI } from "../services/getData"
...
export const BibleProvider: React.FC<BibleProviderProps> = ({ children }) => {
  ...
  const searchBible = async (query: string) => {
    try {
      const response = await axios.get(`https://api.scripture.api.bible/v1/bibles/592420522e16049f-01/search?`, {
        headers: {
          "api-key": API_KEY,
        },
        params: {
  ...
  const getVersesFromPassage = async (passageId: string) => {
    ...
    const [startVerse, endVerse] = range.split("-").map((v) => Number.parseInt(v.split(".")[1]))
    const verses: Verse[] = []
